...
The board will use a little instruction/Enum/Union interpreter (we know that they may have a name of emulator, but since we do not bother do emulate the world phisics we call it's simulation, even they behave like a limited emulator.. we preferer this word as well) on with they are a array of tuple of road number* and instruction on with
...
Infini_Soroban :: struct {
	...
	idx_globa_current_road : i64, // deprecated?
...
Soroban_Operation_Instructions :: struct {
	...
	roads_idx : [MAX_SOROBAN_INSTRUCTIONS_PER_BOARD]i64,
}
